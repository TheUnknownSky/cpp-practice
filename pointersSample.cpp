...
int check_answer(int quest_num){
    ...
    // the variable 'answer' is basically the same as *a; if answer = 'A', then *a = 'A', and vice versa
    int ctr, *c = &ctr; 
    // the variable 'ctr' is basically the same as *c; if ctr = 0, then *c = 0, and vice versa
    *c = 0; 
    // *c or ctr is equal to 0
    cout << "\nAnswer: "; cin >> *a; 
    // answer prompt
    ...
    return *c; 
    // returns the value of ctr or *c; returns 0 if correct, returns 1 if wrong
}
void show_result(int correct, int wrong, int total){
    int *c = &correct, *w = &wrong, *t = &total; 
    // take the values of the parameters to the pointer variables
    *c = *t - *w; 
    // correct = total - wrong
    *t = *c; 
    // total = correct
...
int main(){
    ...
    // the variable 'correct' is basically the same as *c; if correct = 0, then *c = 0, and vice versa
    int wrong = 0, *w = &wrong;     
    // the variable 'wrong' is basically the same as *w; if wrong = 0, then *w = 0, and vice versa
    int total = 3, *t = &total;     
    // the variable 'total' is basically the same as *t; if total = 3, then *t = 3, and vice versa
    question1(); 
    ...
    *w += check_answer(2); 
    // the same thing, but checks the answer of question 2
    question3(); 
    // calls out question 3 function
    *w += check_answer(3); 
    // the same thing, but checks the answer of question 3
    show_result(*c, *w, *t); 
    // takes the values of correct, wrong, and total variables to the function show_result
    main();
